...
export class KafkadminComponent implements OnInit {
  ...
  kafkaAdmin(): any {
    setInterval(() => {
      this.paymentService.kafkaAdmin().subscribe((result: any) => {
        this.pay = result;
  ...
  save(payment: IPayment): void {
    ...
    if (data.approvalStatus === 'Decline' || data.approvalStatus === '') {
      data.approvalStatus = 'Approve';

      this.isSaving = true;
      this.subscribeToSaveResponse(this.paymentService.create(data));
    } else {
      this.isSaving = true;
      this.subscribeToSaveResponse(this.paymentService.create(data));
    }
  ...
  decline(payment: IPayment): void {
    ...
    if (data.approvalStatus === 'Approve' || data.approvalStatus === '') {
  ...
  loadAll(): void {
    this.isLoading = true;
    this.paymentService.queryData().subscribe({
